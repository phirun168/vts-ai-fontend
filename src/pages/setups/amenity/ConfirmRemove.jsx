...
const showAlert = async ({
  ...
  console.log(record, 'test now groupType')

  const getGroupName = (groupId) => {
    console.log(groupId, 'test now groupId')
    const group = groupType?.find((type) => type?._id === groupId)
    return group ? group?.nameEn + ' ' + group?.nameKh : 'Unknown Type'
  }

  const deleteAmenity = async () => {
    try {
      const doc = { _id: record?._id }
      const res = await AmenityServices.removeAmenity({ access_token, doc })
      if (res) {
        getAmenity()
        setLoading(true)
        Swal.fire({
