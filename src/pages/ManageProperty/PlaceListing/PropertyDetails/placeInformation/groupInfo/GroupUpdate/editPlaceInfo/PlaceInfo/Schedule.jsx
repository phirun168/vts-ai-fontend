...
export default function Schedule(props) {
  ...
  const handleAddTimeInterval = ({ days, openTime, closeTime }) => {
    console.log(days, openTime, closeTime, 'add time interval')
  ...
  useEffect(() => {
    if (
      savedSchedule?.days?.Mon?.intervals?.[0]?.openTime ||
      savedSchedule?.days?.Mon?.intervals?.[0]?.closeTime
    ) {
      form?.setFieldsValue({
        checkInOut: [
          toDayjs(savedSchedule?.days?.Mon?.intervals?.[0]?.openTime),
          toDayjs(savedSchedule?.days?.Mon?.intervals?.[0]?.closeTime),
        ],
      })
    } else {
