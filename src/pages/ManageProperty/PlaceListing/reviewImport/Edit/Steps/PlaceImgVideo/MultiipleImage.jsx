...
import { Upload, Button, Input, Radio, Row, Col, message, Checkbox } from 'antd'
import {
  PlusOutlined,
  DeleteOutlined,
  UploadOutlined,
...
export default function MultiipleImage(props) {
  const { keyCat, setKeyCat, onImagesChange, onHandleCheckAddOtherAlbum } =
    props // NEW (callback)
  ...
  //
  const onHandleAddOtherAlbum = (checked) => {
    setShowNewCatInput(checked)
    onHandleCheckAddOtherAlbum?.({ checked: checked })
  }
  ...
  const syncAndNotify = (next) => {
    ...
    // }))
    onImagesChange?.([
      { category_gallery: activekeyCat, gallery: next[activekeyCat] },
    ])
  }
  ...
  return (
    <div>
      ...
      <div className='mb-4 flex items-center gap-2'>
        ...
        <Checkbox
          checked={showNewCatInput}
          onChange={(e) => onHandleAddOtherAlbum(e.target.checked)}
        >
          Add Other Album(Update first)
        </Checkbox>
      </div>
      ...
      <Radio.Group
        ...
        onChange={(e) => {
          ...
          setActivekeyCat(cat)
          // ⚡ tell parent what’s in that category *right now*
