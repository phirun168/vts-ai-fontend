...
export default function PlaceInfoForm(props) {
  ...
  useEffect(() => {
    ...
    /* ---------- activity ---------- */
    const customsAct =
      nonBusinessData.activity?.filter((a) => a.id == null) ?? []
    // const withIdAct =
    //   nonBusinessData.activity
    //     ?.filter((a) => a.id != null)
    //     .map((a) => ({ value: a.id, label: a.nameEn })) ?? []
    ...
    /* ---------- type of place ---------- */
    const customsType =
      nonBusinessData.typeOfPlace?.filter((t) => t.id == null) ?? []
    const withIdType =
    ...
    // console.log(withIdType, 'withIdType me')
        ...
        <Card>
          <Row gutter={[8, 8]}>
            ...
            <Col xs={24} sm={24} md={12} style={{ marginTop: '6px' }}>
              <Form.Item
                name='activity'
                label={`Activity (${dynamicLabelActivity})`}
                hidden={keyCat === 'Rental'}
              >
                <Select
                  mode='multiple'
                  labelInValue // 🔑 add this
                  allowClear
                  showSearch
                  optionFilterProp='label'
                  style={{ width: '100%' }}
